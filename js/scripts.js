...
gsap.to(projectsWrap,{y:pwHeight*-1,ease:"none",scrollTrigger:{trigger:projectsWrap,start:"top top",end:"+="+(window.innerHeight*5),pin:mainWrap,scrub:true,onUpdate:(self)=>{let prog=self.progress*projectsLength,snap=Math.floor(prog),findProj=$('.project_'+snap),findImg='.image_'+findProj.data('index'),path=findProj.data('mask'),bgColor=findProj.data('background-color'),elementsColor=findProj.data('elements-color');project.removeClass('active')
...
function naylaProductRatings(){var rating=$('.product-rating');rating.each(function(){let $this=$(this),stars=$this.find('.product-stars'),perc=$this.data('rating'),i=1;for(i=1;i<=5;i++){stars.append('<span class="material-symbols-outlined">star</span>')}
...
function naylaVideo(){var video=$('.nayla-video');if(video.length){video.each(function(i){i++
...
vid.find('.plyr__video-wrapper').addClass('nayla-video-wrap');vid.find('.plyr__controls').addClass('nayla-controls-wrap');$this.find('.play-icon').on('click',function(){vid.addClass('lightbox-open');naylaVid.restart();})
...
function naylaMarquee(i){var velocity;let marquee=$('.nayla-marquee');marquee.each(function(){let $this=$(this),text=$this.children('*'),dataDuration=$this.data('duration'),seperator=$this.data('seperator');text.append('<span class="material-symbols-outlined">'+seperator+'</span>')
...
naylaMouseCursor();smoothScroll();$(window).on('load',function(){winLoaded=true;window.scrollTo(0,0);naylaVideo();naylaGeneralAnims($('.has-anim'));naylaTextAnims();naylaListAnimations();naylaImageAnims()
...
tl.to('.showcase-footer',{opacity:0},0)}
barba.init({debug:true,cacheIgnore:true,transitions:[{name:'default-transition',leave(){return new Promise(function(resolve,reject){transitions.addClass('running')
...
naylaParallaxImages();initShowcases();initPageElements();naylaSections();initPages();naylaVideo();ScrollTrigger.refresh(true)
setTimeout(function(){let allInstances=ScrollTrigger.getAll();let fixedHeader=ScrollTrigger.getById('fixed_header');allInstances.forEach(function(st){if((st.pin!=null)&&(st.start<=0)){let fixedStart='top+='+st.end+' top';ScrollTrigger.create({trigger:'body',pin:siteHeader,start:'top top',end:fixedStart,id:'header_pin'})}})
